...
public class SignupThree extends JFrame implements ActionListener
{
    ...
    public SignupThree(String formno, String ssn) {
		...
		add(ack2);
		 ...
		 JLabel agencyLabel = new JLabel("Select Agency:");
	        agencyLabel.setFont(new Font("Raleway", Font.BOLD, 20));
	        agencyLabel.setBounds(150, 640, 200, 25);
	        add(agencyLabel);

	        agencyCombo = new JComboBox<>();
	        agencyCombo.setBounds(350, 640, 250, 25);
	        add(agencyCombo);
	     try {
	            BankService service = (BankService) Naming.lookup("rmi://localhost/bankService");
	            List<String> agences = service.getAgencies();
	            for(String ag : agences) {
	                agencyCombo.addItem(ag);
	            }
	        } catch(Exception e) {
	            e.printStackTrace();
	            JOptionPane.showMessageDialog(null, "Error fetching agencies from server!");
	        }
	...
	{		
		...
		{	
			try 
			{
                ...
                int agenceId = Integer.parseInt(selectedAgency.split(" - ")[0]);
                
				 // Appel RMI
			    BankService service = (BankService) Naming.lookup("rmi://localhost/bankService");
			    boolean success = service.signupThree(ssn, accountType, cardNumber, pin, atmCard, chequeBook, internetBanking, mobileBanking, eStatement, emailSMS, agenceId);
